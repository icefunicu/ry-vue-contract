<template>
  <div class="app-container contract-management">
    <el-card class="search-card" shadow="hover">
      <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="80px">
        <el-form-item label="合同标题" prop="title">
          <el-input v-model="queryParams.title" placeholder="请输入合同标题" clearable @keyup.enter.native="handleQuery" />
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="table-card" shadow="hover">
      ...
      <el-table v-loading="loading" :data="contractList" @selection-change="handleSelectionChange" border stripe
        ...
        <el-table-column label="合同内容" align="center" prop="content" width="200">
          <template slot-scope="scope">
        ...
        <el-table-column label="合同状态" align="center" prop="status">
          <template slot-scope="scope">
            <el-tag :type="statusTagType(scope.row.status)" effect="plain" size="medium">
              {{ scope.row.status }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="合同路径" align="center" prop="filePath" />
        <el-table-column label="合同创建时间" align="center" prop="createdTime" width="180">
          <template slot-scope="scope">
            <span>{{ parseTime(scope.row.createdTime, "{y}-{m}-{d}") }}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" class-name="operation-column" width="200">
          <template slot-scope="scope">
            <el-button size="mini" type="primary" icon="el-icon-view" circle :disabled="!(scope.row.status === '待查看')"
              @click="handleUpdate(scope.row)" title="查看"></el-button>
            <el-button size="mini" type="primary" icon="el-icon-check" circle :disabled="!(
          scope.row.status === '草稿' || scope.row.status === '待修改'
        ) || scope.row.createdBy !== userId
        " @click="handleSubmitContract(scope.row)" title="提交"></el-button>
            <el-button size="mini" type="success" icon="el-icon-edit" circle :disabled="!(
          scope.row.status === '草稿' || scope.row.status === '待修改'
        ) || scope.row.createdBy !== userId
        " @click="handleUpdate(scope.row)" title="修改"></el-button>
            <el-button size="mini" type="danger" icon="el-icon-delete" circle :disabled="!(scope.row.status === '草稿' || scope.row.status === '待修改')
    ...
    <el-dialog :title="title" :visible.sync="open" width="1200px" append-to-body custom-class="contract-dialog"
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px" class="contract-form">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="甲方">
              <el-select placeholder="请指定甲方" v-model="form.partyA" @change="handlePartAChange" @focus="getUserList">
          ...
          <el-col :span="8">
            <el-form-item label="乙方">
              <el-select placeholder="请指定乙方" v-model="form.partyB" @change="handlePartBChange">
                <el-option v-for="item in userList" :key="item.userId" :label="item.nickName" :value="item.userId" />
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-divider content-position="left"
          v-if="form.notifyInfoList && form.notifyInfoList.length > 0">双方意见</el-divider>
        ...
        <el-form-item label="意见" v-if="form.status === '待查看' || form.status === '待修改'">
          <el-input v-model="opinion" placeholder="请输入意见" type="textarea" :rows="3" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer contract-dialog-footer">
        <el-button v-if="form.status === '待查看' || form.status === '待修改'" type="primary" class="contract-dialog-button"
          @click="handleOpinion(form.id)">提交意见</el-button>
        <el-button type="primary" icon="el-icon-check" @click="submitForm"
          v-if="userId !== form.createdBy || form.status !== '待查看'">确 定</el-button>
...
export default {
  ...
  // 修改 beforeDestroy 方法
  beforeDestroy() {
    // 不再需要销毁编辑器，vue-quill-editor会自动处理
  },
  created() {
